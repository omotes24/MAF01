...
: "${SAVE_ROOT:=${HOME_ROOT}/260422}"
: "${BACKBONES:=imagenet_vit bioclip}"
: "${SEEDS:=42 123 456}"
: "${CUDA_VISIBLE_DEVICES:=0}"
: "${TMUX_SESSION:=maf-geometry-alpha-gpu0-260422}"
: "${OUTPUT_SUBDIR:=geometry_alpha_study_summary}"
...
: "${LOG_PATH:=${SAVE_ROOT}/logs/maf_geometry_alpha_gpu${CUDA_VISIBLE_DEVICES}.log}"
...
RUN_CMD="${RUN_CMD} && \
CUDA_VISIBLE_DEVICES='${CUDA_VISIBLE_DEVICES}' python run_multiseed_adaptive_study.py \
--data-src '${DATA_SRC}' \
--save-root '${SAVE_ROOT}' \
