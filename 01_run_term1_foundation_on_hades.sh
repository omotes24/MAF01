...
: "${BACKBONES:=imagenet_vit openai_clip_b16 openai_clip_b32 bioclip dinov2_vitb14 resnet50 swin_base}"
: "${SEEDS:=42 123 456 789 1011}"
: "${CUDA_VISIBLE_DEVICES:=0}"
: "${TMUX_SESSION:=maf-term1-foundation-gpu0}"
: "${OUTPUT_SUBDIR:=term1_foundation_summary}"
: "${OFFICIAL_TRACK_I_CSV:=${SAVE_ROOT}/official_track_i_results_multi_backbone.csv}"
: "${INCLUDE_APPROX_TRACK_I:=1}"
: "${EVAL_ONLY:=0}"
: "${FORCE_REEXTRACT:=0}"
: "${INSTALL_DEPS:=0}"
: "${TORCH_VERSION:=2.6.0}"
...
: "${LOG_PATH:=${SAVE_ROOT}/logs/maf_term1_foundation_gpu${CUDA_VISIBLE_DEVICES}.log}"
...
RUN_CMD="${RUN_CMD} && \
CUDA_VISIBLE_DEVICES='${CUDA_VISIBLE_DEVICES}' python 01_run_term1_foundation.py \
--data-src '${DATA_SRC}' \
--save-root '${SAVE_ROOT}' \
...
if [[ "${INCLUDE_APPROX_TRACK_I}" == "1" ]]; then
  RUN_CMD="${RUN_CMD} --include-approx-track-i"
fi
...
echo "Seeds       : ${SEEDS}"
echo "Approx T1   : ${INCLUDE_APPROX_TRACK_I}"
